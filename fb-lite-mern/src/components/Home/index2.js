import React, { useState, useEffect } from "react";
import _ from 'lodash';
import auth from "../auth";
// import { Link } from "react-router-dom";
import axios from "axios";
var moment = require("moment");

// import MemberContext from "../Contexts/MemberContext"

const HomeContainer = props => {
  const [memberInfo, setMemberInfo] = useState(props.location.state.memberInfo); 
  // userInfo
  const [friends, setFriends] = useState(
    props.location.state.memberInfo.friends
  ); // userInfo
  const [friendReqs, setFriendReqs] = useState(
    props.location.state.memberInfo.friendRequests
  );
  const [friendRequested, setFriendRequested] = useState(
    props.location.state.memberInfo.friendRequested
  );
  const [posts, setPosts] = useState([]);
  const [postQuote, setPostQuote] = useState({});
  const [allMembers, setAllMembers] = useState([]);

  useEffect(() => {
    getAllMembers();
    // getPosts();

  }, []);

  const getAllMembers = () => {
    let friendsObjectCreate = [];
    axios
      .get("http://localhost:5000/members/")
      .then(res =>{
        // console.log("get All members"+ JSON.stringify(res.data));
        const allMembers = res.data;

        setAllMembers(allMembers);
        // setFriends(friendsObjectCreate)



      })
      .catch(erro => {
        console.log("get posts error for user: " + erro);
      });
  };
  const logout = () => {
    auth.logout(() => {
      props.history.push("/login");
    });
  };

  const getPosts = () => {

    const getPosts = {
      friends: friends
    };

    axios
      .post("http://localhost:5000/posts/getRelatedPosts", getPosts)
      .then(res => {

        // console.log("get my friends-> get their posts-> get my posts-> get users which are public and their posts "+ JSON.stringify(res.data));
        setPosts(res.data);
      })
      .catch(erro => {
        console.log("get posts error for user: " + erro);
      });
  };

  const onChangeInputPost = e => {
    setPostQuote(e.target.value);
  };

  const PostQuotePostQuote = e => {
    e.preventDefault();
    console.log("Post text" + postQuote + " for user  " + memberInfo._id);

    const post = {
      body: postQuote,
      authorid: memberInfo._id,
      authorname: memberInfo.screenName
    };

    axios
      .post("http://localhost:5000/posts/addPost", post)
      .then(res => {
        if (res.statusText === "OK") {
          getPosts();
          // auth.login(() => {
          //   props.history.push("/admin");
          // });
        }

      })
      .catch(erro => {
        console.log("shoud redirect to register| Details: :" + erro);
      });
  };

  const searchFriend = () => {
    // console.log(memberInfo);
    // console.log(allMembers);
    console.log(friendRequested);
  };

  const acceptRequest = objectId => {
    console.log("acept request for friend ..." + objectId);

    const acceptRequest = {
      _id: objectId,
      _ownerId: memberInfo._id,
      currentFriends: friends,
      friendRequests: friendReqs,
      friendRequested: friendRequested
    };

    axios
      .post("http://localhost:5000/members/acceptFriend", acceptRequest)
      .then(res => {
        console.log(" The response after accept a friend");
        getAllMembers();
      })
      .catch(erro => {
        console.log("erro : :" + erro);
      });
  };

  const AddFriend = objectId => {
    console.log("friend ..." + objectId + " has been added");
    // const friend = {
    //   object_id: memberInfo._id,
    // };

    console.log("something else")
    const request = {
      _id: objectId,
      _ownerId: memberInfo._id,
      currentFriends: friends,
      friendRequests: friendReqs,
      friendRequested: friendRequested
    };
    axios
      .post("http://localhost:5000/members/addFriend", request)
      .then(res => {
        console.log(" The response after add a friend");
       console.log(res.data)
        // setAllMembers([...allMembers, res.data])
        getAllMembers();

        // console.log(res.data);
        // console.log(allMembers)
        // setFriendRequested(oldArray => [...oldArray, newElement]);  
        
        // allMembers.forEach(function(member) {
        //     if (res.data._id ===  member._id)
        //     console.log(res.data)
        //     setAllMembers([...allMembers, res.data])
        //     // setAllMembers([...allMembers, result])
        // });
        // update  replace objects id in with updated version
        

        // getAllMembers();
        // setFriendRequested(oldArray => [...oldArray, newElement]);
        // setAllMembers(res.data)
        // console.log(allMembers);

      })
      .catch(erro => {
        console.log("erro : :" + erro);
      });
  };

  return (
    <div>
      <h1>Profile Details</h1>
      {[memberInfo].map(member => (
        <div key={member.objectId}>
          <div>{member._id}</div>
          <div>{member.screenName}</div>
          <div>{member.email}</div>
          <div>{member.visibility}</div>
          <div>{member.password}</div>
          {/* <div>{member.friends}</div> */}
        </div>
      ))}

      <hr></hr>
      <h1>Friend Search</h1>

      {allMembers.map(member =>
        // friends.map(friend => (
        <div>
        { member._id !== memberInfo._id  && 
         !friends.some( friend => friend === member._id) && 
         !friendReqs.some( friend => friend === member._id) &&
         !friendRequested.some( friend => friend === member._id) &&
         <div>{member.screenName}{" "} <button onClick={() => AddFriend(member._id)}>
          Add Friend
        </button></div>}
        { member._id !== memberInfo._id  && 
         friends.some( friend => friend === member._id) && 
         !friendReqs.some( friend => friend === member._id) &&
         !friendRequested.some( friend => friend === member._id) &&
         <div>{member.screenName} {" "}
          Is Already a Friend
        </div>}
        { member._id !== memberInfo._id  && 
         !friends.some( friend => friend === member._id) && 
         friendReqs.some( friend => friend === member._id) &&
         !friendRequested.some( friend => friend === member._id) &&
         <div>{member.screenName} <button onClick={() => acceptRequest(member._id)} >
          Confirm
        </button>
        {/* <button >
          Reject
        </button> */}
        </div>}
        { member._id !== memberInfo._id  && 
         !friends.some( friend => friend === member._id) && 
         !friendReqs.some( friend => friend === member._id) &&
         friendRequested.some( friend => friend === member._id) &&
          <div>{member.screenName}<button disabled>
          Pending Friend Request
        </button></div>}
      
      </div>
// ))
      )}

      <hr></hr>
      <h1>Friend Requests</h1>
      {allMembers.map(member => (
        <div>
     { member._id !== memberInfo._id  && 
      !friends.some( friend => friend === member._id) && 
      friendReqs.some( friend => friend === member._id) &&
      !friendRequested.some( friend => friend === member._id) &&
      <div>{member.screenName} <button onClick={() => acceptRequest(member._id)} >
       Confirm
     </button>
     <button >
       Reject
     </button>
     </div>}
     </div>



      ))}
      

      <hr></hr>
      <h1>Posts</h1>
        <textarea
          name="inputPost"
          onChange={onChangeInputPost}
          rows="4"
          cols="50"
        ></textarea>
        <button onClick={PostQuotePostQuote}>Post</button>
     

      {posts.map(post => (
        <div style={{ borderColor: "red", borderStyle: "solid" }}>
          <div>{JSON.stringify(post.body)}</div>
          <div>by: {JSON.stringify(post.authorname)}</div>
          <div>At: {moment(post.createdAt).format(" HH:mm DD-MM-YYYY")}</div>
        </div>
      ))}
      {/* { posts &&
      <h1> posts are loaded {posts.data.body}</h1>
      // [posts].map(post => ( 
      //   <div>{post}</div>
      //   ))
      } */}
      {/* {[posts].map(post => ( 
      <div>{post}</div>
      ))} */}
      <button onClick={logout}>Log out</button>
      <button onClick={searchFriend}>search Friend</button>
    </div>
  );
};

export default HomeContainer;
